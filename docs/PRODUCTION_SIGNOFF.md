...
| ☐   | P27 | `FloatingActionButton` and `SearchBar` in `src/components/ui/`. No root-level import paths for either.                    | ✅ P-27 — both moved to `ui/`; 7 import paths updated; `BottomSheetForm` has no active importers as of v3.6 (ProductsScreen migrated to ConfirmModal) | Cross    |
...
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| M-01      | Modal library not fully consolidated — `NewProductModal` migrated off `react-native-modal` in v3.6; `NewCustomerModal` / `NewSupplierModal` still use `AppModal`. Full consolidation deferred. | Non-breaking; full migration is a larger refactor |
...
## UI Redesigns Checklist (v3.6)

| #   | ID  | Requirement                                                                                                                                                | Code Status                                                              | Category |
| --- | --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | -------- |
| ☐   | P39 | ExportScreen: date presets (All time / This month) populate From/To inputs correctly. Manual edit clears preset chip.                                      | ✅ v3.6 — `applyPreset()` + `setDatePreset(null)` on manual input change | Screen   |
| ☐   | P40 | ExportScreen: tapping an Export CSV pill triggers loading state. Concurrent taps on other rows are disabled.                                               | ✅ v3.6 — `loadingKey` state disables all rows while one is loading      | Screen   |
| ☐   | P41 | ProfileScreen: business mode segment (Seller / Both / Distributor) updates `dashboard_mode` in DB and persists after app restart.                          | ✅ v3.6 — `updateField("dashboard_mode", v)` on segment change           | Screen   |
| ☐   | P42 | ProfileScreen: "Sign Out" triggers confirmation alert before logout. Tapping Cancel does not log out.                                                      | ✅ v3.6 — `Alert.alert` with Cancel / Sign Out actions in ProfileScreen  | Screen   |
| ☐   | P43 | ProductsScreen: delete product taps open ConfirmModal. Confirming deletes the product and refreshes the list.                                              | ✅ v3.6 — `ConfirmModal` + `handleRequestDelete` + `handleConfirmDelete` | Screen   |
| ☐   | P44 | Financial Position screen: Customers Owe Me (green), I Owe Suppliers (pinkish-red #E0336E), Net Card (dark #1C2333) all render correct colors and amounts. | ✅ v3.6 — StatCard + NetCard sub-components with correct color props     | Screen   |
