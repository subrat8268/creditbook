...
### ✅ P-21 — `ProfileScreen` Avatar, Mode Toggle, Sign Out + `ExportScreen` Back Button — FIXED March 8, 2026

**Files:** `src/screens/ProfileScreen.tsx`, `src/screens/ExportScreen.tsx`  
**Category:** UX correctness + navigation

- **Avatar**: Added `getInitials()` helper; 72dp filled `#22C55E` circle with white bold 22px initials rendered as a centered header above form fields
- **Mode toggle**: `["vendor","receiver"]` (wrong) → `[{"seller","Seller"},{"distributor","Distributor"},{"both","Both"}]` — correct values and correct left→right order; active check uses `profile.dashboard_mode ?? "seller"`
- **Mode dual-write**: Already works via `updateField` which calls both Supabase `.update()` and `setProfile({ ...current, [field]: value })`; no additional change needed
- **Sign Out**: `onPress={logout}` → `handleSignOut` async: `supabase.auth.signOut()` + `logout()` + `router.replace("/(auth)/login")`
- **ExportScreen back button**: Added `ArrowLeft` + `useRouter`; `<TouchableOpacity onPress={() => router.back()} hitSlop={{...}}><ArrowLeft size={22} color="#1C1C1E" strokeWidth={1.75}/></TouchableOpacity>` above title text
...
### ✅ P-23 — FlatList Performance Audit — FIXED March 8, 2026

**Files:** `src/components/customers/CustomerList.tsx`, `src/components/suppliers/SupplierList.tsx`, `src/components/orders/OrderList.tsx`, `src/screens/ProductsScreen.tsx`

All four active FlatList surfaces now carry the full perf prop set:

| File                 | `ITEM_H` | Props added                                                                                                             |
| -------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------- |
| `CustomerList.tsx`   | 88dp     | `useCallback` renderItem, `maxToRenderPerBatch={10}`, `windowSize={5}`, `getItemLayout`, `removeClippedSubviews={true}` |
| `SupplierList.tsx`   | 80dp     | same                                                                                                                    |
| `OrderList.tsx`      | 88dp     | same + `initialNumToRender={10}`, `contentContainerStyle={{ paddingBottom: 80 }}`                                       |
| `ProductsScreen.tsx` | 72dp     | same as OrderList                                                                                                       |

`CustomerDetail` transaction feed uses `ScrollView` + `.map()` (variable-height rows) — no FlatList, no changes needed.
